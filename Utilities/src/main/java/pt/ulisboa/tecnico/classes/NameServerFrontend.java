...
public class NameServerFrontend {
  ...
  private final HashMap<String, Integer> _cachedInactiveServers;
  ...
  public List<ClassServerNamingServer.ServerEntry> list() {
    return this._stub
        .list(ClassServerNamingServer.ListRequest.getDefaultInstance())
        .getServersList();
  }
  ...
  private List<ClassServerNamingServer.ServerEntry> getLookupServerList(
    ...
    List<ClassServerNamingServer.ServerEntry> servers =
        response.getServersList().stream()
  ...
  public ClassServerNamingServer.ServerEntry lookup(
    ...
    List<ClassServerNamingServer.ServerEntry> servers =
        this.getLookupServerList(serviceName, qualifiers);
  ...
  public ManagedChannel getChannel(
    ...
    if (_cachedChannel == null
        || (!_cachedServer.getQualifiersList().containsAll(qualifiers) && !qualifiers.isEmpty())
        || previousIsInactive) {
      if (_cachedChannel != null) {
      ...
      _cachedChannel =
          ManagedChannelBuilder.forTarget(_cachedServer.getAddress()).usePlaintext().build();
    }
