...
public class ProfessorFrontend extends TimestampsManager {
   ...
   * @param capacity
   */
  public ResponseCode openEnrollmentsCommand(int capacity)
      throws ResponseException, StatusRuntimeException {
    ProfessorRPCOpenEnrollments rpcCall =
        new ProfessorRPCOpenEnrollments(List.of("P"), this._nameServer);
  ...
  /**
   * Sends a closeEnrollments request to the server and changes the class state to not allow further
   * enrollments, prints the response code
   */
  public ResponseCode closeEnrollmentsCommand() throws ResponseException, StatusRuntimeException {
    ProfessorRPCCloseEnrollments rpcCall =
        new ProfessorRPCCloseEnrollments(List.of("P"), this._nameServer);
  ...
  /**
   * Sends a cancelEnrollment request to the server and changes the class state un enrolling a
   * student in the class, prints the response code
   *
   * @param studentId
   */
  public ResponseCode cancelEnrollmentCommand(String studentId)
      throws ResponseException, StatusRuntimeException {
    ProfessorRPCCancelEnrollment rpcCall =
        new ProfessorRPCCancelEnrollment(List.of("P"), this._nameServer);

    rpcCall.setStudentId(studentId);
