...
public class ProfessorService extends ProfessorServiceGrpc.ProfessorServiceImplBase {
  ...
  /**
   * "openEnrollments" remote procedure call. Receives openEnrollmentRequest from the professor
   * client changes the class state and sends the adequate response code through a StreamObserver
   *
   * @param request
      ...
      synchronized (this._classObj) {
        if (this._classObj.getClassState().getOpenEnrollments()) {
          ...
          ClassesDefinitions.ClassState.Builder classStateBuilder =
              this._classObj.getClassState().toBuilder();
  ...
  /**
   * "closeEnrollments" remote procedure call. Receives closeEnrollmentRequest from the professor
   * client changes the class state and sends the adequate response code through a StreamObserver
   *
   * @param request
   * @param responseObserver
  ...
  /**
   * "list" remote procedure call. Receives listRequest from the professor client and sends the
  ...
  public void listClass(
    ...
    if (!_properties.get("isActive")) {
      ...
      while (this._replicaManager
          .getTimestampsManager()
          .isTimestampMostUptoDate(request.getTimestampsMap())) {
        try {
  ...
  /**
   * "cancelEnrollment" remote procedure call. Receives cancelEnrollmentRequest from the professor
   * client changes the class state and sends the adequate response code through a StreamObserver
   *
   * @param request
   * @param responseObserver
    ...
    if (!_properties.get("isActive")) {
      ...
      synchronized (this._classObj) {
        ...
        ClassesDefinitions.Student studentToDiscard =
            currentClassState.getEnrolledList().stream()
        ...
        if (studentToDiscard != null) {
          ...
          classStateBuilder.removeEnrolled(
              currentClassState.getEnrolledList().indexOf(studentToDiscard));
