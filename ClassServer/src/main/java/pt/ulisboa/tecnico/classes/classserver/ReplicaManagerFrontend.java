...
public class ReplicaManagerFrontend {
  ...
  public ReplicaManagerFrontend(
    ...
    this._previousTimestamps =
        new HashMap<String, Integer>(this._timestampsManager.getTimestamps());
  }
  ...
  public void updateLocalTimestamp() {
    this._timestampsManager.putTimestamp(
        this._address, this._timestampsManager.getTimestamps().get(this._address) + 1);
    LOGGER.info(
  ...
  private void propagate(ClassServerNamingServer.ServerEntry server) {
    ReplicaManagerClassServer.PropagateStatePushRequest.Builder request =
        ReplicaManagerClassServer.PropagateStatePushRequest.newBuilder();
    ...
    ReplicaManagerClassServer.PropagateStatePushResponse response =
        _stub.propagateStatePush(request.build());
    ...
    LOGGER.info(
        "[ReplicaManager Frontend] Propagated Timestamps: "
            + this._timestampsManager.getTimestamps());
  }
  ...
  public void propagateStatePush(boolean forceGossip) {
    if ((!this._previousTimestamps.equals(this._timestampsManager.getTimestamps())
            && _properties.get("isActive"))
        || forceGossip) {
